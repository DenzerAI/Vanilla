...
type HeadingProps = {texts:string[];reduceMotion?:boolean;paused?:boolean;pauseAdvance?:boolean};
export function ChatStartHeading(props:HeadingProps) {
 return <StartSpeech key={props.texts.join('\n')} {...props}/>;
}
function StartSpeech({texts,reduceMotion=false,paused=false,pauseAdvance=false}:HeadingProps) {
 ...
 useEffect(()=>{
  ...
  if(!onscreen || !pageVisible || paused){setFrame(old=>({...old,fading:false}));return;}
  ...
  const tick=()=>{
   ...
   else if(!pauseAdvance && current.index<texts.length-1) timer=setTimeout(()=>{setFrame({...current,fading:true});timer=setTimeout(()=>{current={index:(current.index+1)%texts.length,count:0,fading:false};setFrame(current);timer=setTimeout(tick,chatHeadingMotion.character);},chatHeadingMotion.fade);},chatHeadingMotion.hold);
  };
