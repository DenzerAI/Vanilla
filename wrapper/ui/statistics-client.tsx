...
export function useStatisticsData(api:any,projectId:string,revision:string,live=false,enabled=true) {
 ...
 useEffect(()=>{if(!api||!enabled)return;let alive=true,pending=false;const load=async()=>{if(pending||document.hidden)return;pending=true;try{const data=await api('/statistics?projectId='+encodeURIComponent(projectId)+'&timeZone='+encodeURIComponent(statisticsTimeZone()));if(alive)setState({projectId,data});}catch{if(alive)setState({projectId,data:{error:true}});}finally{pending=false;}};void load();const timer=live?setInterval(load,15000):null;const visible=()=>{if(!document.hidden)void load();};window.addEventListener('focus',visible);document.addEventListener('visibilitychange',visible);return()=>{alive=false;if(timer)clearInterval(timer);window.removeEventListener('focus',visible);document.removeEventListener('visibilitychange',visible);};},[api,projectId,revision,live,enabled]);
