...
import { timestamp, workingDurationLabel, activityKind, activityLabel, activitySummary } from './chat-presentation.mjs';
...
export function ActivityGroup({items, running, children, workerId}) {
  ...
  const showSummary = active || successful.some(i => i.type !== 'reasoning') || !failed;
  ...
  return <details className="activity-group" onToggle={event => {if (event.target === event.currentTarget) setOpen(event.currentTarget.open);}}>
    <summary>
      ...
      <span className="activity-summary">
        {showSummary ? active ? activityLabel(active, true) : activitySummary(successful) : null}
        {!!failed && <span className="activity-failed">{showSummary && ' · '}{failed === 1 ? 'Ein Schritt fehlgeschlagen' : `${failed} Schritte fehlgeschlagen`}</span>}
      </span>
...
export function TurnStatus({ turn, running, waiting, visible = true, hasAnswer = false }) {
  ...
  if (!running && elapsed == null && hasAnswer && turn.status === 'completed') return null;
