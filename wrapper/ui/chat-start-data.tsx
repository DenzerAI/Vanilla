...
export function useChatStartData(api:any,enabled:boolean,revision:number) {
 const [data,setData]=useState({jobs:[] as any[],entries:[] as any[],reports:[] as any[],userProfile:{name:'',location:''},profileError:false,error:'',loaded:false});
 useEffect(()=>{
  if(!api)return;
  ...
  const load=async()=>{
   ...
   setData(old=>({loaded:true,userProfile:results[3].status==='fulfilled'?readUserProfile(results[3].value.text):old.userProfile,profileError:results[3].status==='rejected',jobs:results[0].status==='fulfilled'?results[0].value:old.jobs,entries:results[1].status==='fulfilled'?results[1].value.entries||[]:old.entries,reports:results[2].status==='fulfilled'?results[2].value.items||[]:old.reports,error:results.some(r=>r.status==='rejected')?'Einige Tagesinhalte konnten nicht aktualisiert werden.':''}));
  };
