...
export function useChatStartData(api:any,enabled:boolean,revision:number) {
 const [data,setData]=useState({jobs:[] as any[],entries:[] as any[],reports:[] as any[],error:''});
 useEffect(()=>{
  if(!api)return;
  ...
  const load=async()=>{
   ...
   setData(old=>({jobs:results[0].status==='fulfilled'?results[0].value:old.jobs,entries:results[1].status==='fulfilled'?results[1].value.entries||[]:old.entries,reports:results[2].status==='fulfilled'?results[2].value.items||[]:old.reports,error:results.some(r=>r.status==='rejected')?'Einige Tagesinhalte konnten nicht aktualisiert werden.':''}));
  };
