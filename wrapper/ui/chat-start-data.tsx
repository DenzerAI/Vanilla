...
export function useChatStartData(api:any,enabled:boolean,revision:number) {
 ...
 useEffect(()=>{
  ...
  const load=async()=>{
   ...
   const results=await Promise.allSettled([api('/jobs'),api('/library'),enabled?api('/planner/results'):Promise.resolve({items:[]}),api('/file/text?path='+encodeURIComponent(userProfilePath))]);
   pending=false;if(!alive)return;
   const profile=results[3].status==='fulfilled'?readUserProfile(results[3].value.text):null;
   ...
   setData(old=>({weather:profile&&JSON.stringify(profile)===JSON.stringify(old.userProfile)?old.weather:{status:'loading'},loaded:true,userProfile:results[3].status==='fulfilled'?readUserProfile(results[3].value.text):old.userProfile,profileError:results[3].status==='rejected',jobs:results[0].status==='fulfilled'?results[0].value:old.jobs,entries:results[1].status==='fulfilled'?results[1].value.entries||[]:old.entries,reports:results[2].status==='fulfilled'?results[2].value.items||[]:old.reports,error:results.some(r=>r.status==='rejected')?'Einige Tagesinhalte konnten nicht aktualisiert werden.':''}));
  };
