import {SpeechPlayback, spokenText, onMicrophoneChange, microphoneBusy} from './speech-playback.mjs';
...
export class ChatAudio {
  constructor({playerFactory=(api,changed)=>new SpeechPlayback(api,changed), pauseMs=300000}={}) {
    ...
    this.state={}; this.queue=[]; this.seen=new Set(); this.revision=0;
  }
  subscribe=(fn)=>{this.listeners.add(fn);return()=>this.listeners.delete(fn);};
  snapshot=()=>this.state;
  emit(patch) { this.state={...this.state,...patch}; for(const fn of this.listeners) fn(); }
  stop=()=>{
    this.revision++; clearTimeout(this.timer); this.queue=[]; this.seen.clear();
  ...
  async start(api,{chatId,title,mode='message',key,text}) {
    ...
    this.player.onReplaced=this.stop;
