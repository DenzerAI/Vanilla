...
export function AIMaintenanceSettings({api,section="all"}:{api:(path:string,body?:unknown)=>Promise<any>;section?:"all"|"catalog"|"updates"}) {
 ...
 return <section className="worker-settings" aria-label="KI-Aktualisierungen">
  ...
  {section!=="catalog"&&<details className="settings-group ai-disclosure"><summary>Aktualisierungen</summary>
  ...
  {section!=="updates"&&<details className="ai-disclosure"><summary>Weitere Anbieter & Modelle</summary>
  ...
  <div className="settings-group">{data?.items.filter(item=>`${item.name} ${item.program||''} ${(item.models||[]).join(' ')} ${(item.publicModels||[]).map(m=>m.name).join(' ')}`.toLowerCase().includes(query.toLowerCase())).map(item=><React.Fragment key={item.id}>
   <SettingRow icon={<BrandIcon name={item.id}/>} title={item.name} description={`${item.kind==='local'?'Auf deinem Rechner':item.kind==='agent'?'KI-Programm':'Cloud-KI'}${item.program?' · '+item.program:''} · ${item.phase?phases[item.phase]:'Öffentlicher Katalog'}${item.publicModels?.[0]?' · Im Katalog: '+item.publicModels[0].name:''}`} action={<a className="button" href={item.source} target="_blank" rel="noreferrer">Beim Anbieter ↗</a>}/>
