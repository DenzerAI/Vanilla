...
function App({ embedded = false, sessionRef, onSessionChange, onActivate, paneNumber = 0, showPaneHeader = false, isMaximized = false, onMaximize, onClosePane, onOpenFile, paneVisible = true, initialProject = "default" }) {
  ...
  // The composer picks the worker for the next chat; an open chat stays with its own worker.
  const engineChoice = engine || boot?.workerSettings?.defaultWorker || "auto",
    composerWorker = current?.workerId || (engineChoice === "auto" ? boot?.effectiveWorker : engineChoice) || "codex",
    composerModels = current?.models?.length ? current.models : boot?.modelsByWorker?.[composerWorker] || [];
  function chooseEngine(id) {
    setEngine(id);
    const list = boot?.modelsByWorker?.[id === "auto" ? boot?.effectiveWorker : id] || [];
    setModel(list.find((m) => m.isDefault)?.model || list[0]?.model || "");
  }
              ...
              <div className="composer-area">
                ...
                <form className={"composer pill-composer " + (mode === "plan" ? "planning" : "")} onSubmit={guard(submit)}>
                  ...
                  <div className="composer-options" role="group" aria-label="Nachrichtenoptionen">
                      ...
                      <ChatMenu
                        label={current ? `Worker dieses Chats: ${workerName(current.workerId)}` : `Worker: ${workerName(engineChoice)}`}
                        className="mode-trigger"
                        disabled={!!current || running || busy}
                        placement="above"
                        selected={current ? current.workerId : engineChoice}
                        items={[
                          { id: "auto", label: "Automatisch", icon: icon(BrainCircuit, 16), action: () => chooseEngine("auto") },
                          ...(boot.workers || []).filter((w) => w.configured || w.id === current?.workerId).map((w) => ({ id: w.id, label: w.name, detail: w.connected ? undefined : w.status, icon: icon(BrainCircuit, 16), action: () => chooseEngine(w.id) })),
                        ]}
                      >
                        <span>{workerName(current ? current.workerId : engineChoice)}</span>
                        {icon(ChevronDown, 14)}
                      </ChatMenu>
                      <ModelPicker
