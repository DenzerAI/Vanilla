...
function ChatTurn({ turn, running, waiting, visible, actionsDisabled, paneNumber = 0, workerId, ...actions }) {
  ...
  const messages = groups.filter(group => group.type === "message");
  ...
  return <section className="chat-turn" id={`pane-${paneNumber}-turn-${turn.id}`} tabIndex={-1} aria-label="Nachricht und Antwort">
    {messages.map(group => <Item key={group.id} item={group.item} workerId={workerId} {...actions} running={actionsDisabled} sentAt={turn.startedAt} completedAt={!running && group.item.id === finalMessage?.id ? turn.completedAt : null} />)}
    <TurnStatus turn={turn} running={running} waiting={waiting} visible={visible} hasAnswer={!!finalMessage} />
    {!!activity.length && <ActivityGroup items={activity} running={running} workerId={workerId} turn={turn}>
      {activity.map(item => <Item key={item.id} item={item} workerId={workerId} {...actions} running={running} />)}
    </ActivityGroup>}
...
function App({ embedded = false, sessionRef, onSessionChange, onActivate, paneNumber = 0, showPaneHeader = false, isMaximized = false, onMaximize, onClosePane, onOpenFile, paneVisible = true, initialProject = "default" }) {
  ...
  useEffect(()=>{if(embedded||view!=="inbox")return;const previous=sidebar;setSidebar(true);return()=>setSidebar(previous);},[view,embedded]);
  const mailHelp = value => { setText(t=>(t?t+'\n\n':'')+value);setView('chat');setModal(null);requestAnimationFrame(()=>inputRef.current?.focus()); };
  ...
  const nav = [
      ...(boot?.features?.mailInbox?[["inbox", Mail, "Inbox"]]:[]),
      ["jobs", Clock, "Aufträge"],
  ...
  return (
    ...
    >
      {!embedded && <aside className="sidebar">
        ...
        {view === "inbox" ? <div className="inbox-sidebar-slot" ref={setInboxSidebarHost}/> : view === "settings" ? (
          <>
      ...
      <MainSurface className="main">
        ...
        {view === "chat" ? null : view === "inbox" ? (
          <InboxPage key={projectId} api={api} projectId={projectId} PageHeading={PageHeading} sidebarHost={inboxSidebarHost} sidebarVisible={sidebar} onShowSidebar={()=>setSidebar(true)} onHideSidebar={()=>setSidebar(false)} onBack={()=>setView('chat')} onConnections={()=>setView('connections')} onAgent={mailHelp}/>
        ) : view === "jobs" ? (
          <div className="page">
          ...
          <div className="page connections-page">
            ...
            <ConnectionsContent api={api} features={boot.features} integrations={{...integrations,mailProject:projectId}} audioConnections={audioConnections}
      ...
      {modal?.type==='mail-connection'&&<Modal title={modal.connection.id?'Verbindung bearbeiten':'Verbindung hinzufügen'} onClose={()=>setModal(null)}><MailConnectionForm connection={modal.connection} api={api} projectId={projectId} onHelp={mailHelp} onSaved={async()=>{setIntegrations(await api('/integrations'));setModal(null);setView('inbox');}}/></Modal>}
