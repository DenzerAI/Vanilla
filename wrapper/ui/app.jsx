...
function App({ embedded = false, sessionRef, onSessionChange, onActivate, paneNumber = 0, showPaneHeader = false, isMaximized = false, onMaximize, onClosePane, onOpenFile, paneVisible = true, initialProject = "default" }) {
  ...
  const settingsCatalog = view === 'connections' || view === 'skills';
  const openSetting = id => {
    if (id === 'connections' || id === 'skills') setView(id);
    else { setSettingsTab(id); setView('settings'); }
  };
  const settingNav = [
            ...
            <nav>
              {settingNav.map(([id, I, label]) => (
                <button
                  key={id}
                  className={
                    "nav-item " + ((settingsCatalog ? view : settingsTab) === id ? "selected" : "")
                  }
              ...
              <div className="composer-area">
                ...
                <form className={"composer pill-composer " + (mode === "plan" ? "planning" : "")} onSubmit={guard(submit)}>
                  ...
                  <div className="composer-options" role="group" aria-label="Nachrichtenoptionen">
                      ...
                      {thread?.workerSession?.configOptions === undefined && <ModelPicker
                        models={thread?.workerSession?.models ? thread.workerSession.models.availableModels.map(m => ({model:m.modelId,displayName:m.name || m.modelId,supportedReasoningEfforts:[]})) : current?.models?.length ? current.models : boot.modelsByWorker?.[current?.workerId || boot.effectiveWorker || "codex"] || []}
                        model={model}
                        effort={effort}
        ...
        {view === "chat" ? null : view === "inbox" ? (
          ...
          <div className={"page settings-page" + (settingsTab === "identity" ? " agent-settings-page" : "")}>
            ...
            {['system','memory','storage','access'].includes(settingsTab) && boot.features.operations ? <SystemSettings key={settingsTab} api={api} section={settingsTab} chats={chats} onJobs={()=>setView('jobs')} onLibrary={()=>setView('library')} onConnections={()=>setView('connections')}/> : settingsTab === "general" ? (
              ...
              boot.features.privacy ? <PrivacySettings api={api} onSettings={setSettingsTab} onSearch={()=>{setSearch("");setModal("search");}} /> : <div className="settings-group"><SettingRow title="Datenschutzmodul noch nicht geladen" description="Die neue Vorprüfung wird nach einem regulären Serverneustart verfügbar. Laufende Arbeit zuerst abschließen; die Schutzregeln sind bis dahin noch nicht aktiv."/></div>
            ) : settingsTab === "engines" ? (
              <>
      ...
      {modal === "search" && (
        <Modal title="Suche" className="system-search-dialog" onClose={() => setModal(null)}>
          <SystemSearch api={api} pages={[
            ...
            if (result.kind === "setting") { openSetting(result.id); return; }
            setView(result.id);
          })}/>
        </Modal>
      )}
