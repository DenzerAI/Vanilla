import {AllowanceDashboard} from './usage';
...
export function StatisticsPreview(){
 ...
 return <><p className="page-note">Beispieldaten · Glaskarten. Wischen, horizontal scrollen oder Pfeile verwenden.</p><AttentionFan items={[{id:'allowances',kind:'allowances',title:'Kontingente',description:'Beispieldaten',allowances:allowanceExample},{id:'statistics',kind:'statistics',title:'Statistik',description:'Beispieldaten',statistics:statisticsExample},{id:'weather',kind:'weather',title:'Dein Wetter',description:'Beispiel einer Wetterkarte'},{id:'answer',kind:'chat',title:'Dein Entwurf ist fertig',description:'Die überarbeitete Fassung liegt zum Lesen bereit.'},{id:'calendar',kind:'prompt',title:'Dein Kalender',description:'Beispiel einer zukünftigen Kalenderkarte.'},{id:'job',kind:'job',title:'Dein Tagesüberblick',description:'Beispiel eines geplanten Auftrags.'}]} onOpen={item=>{if(['statistics','allowances'].includes(item.kind))setOpen(item.kind);}}/>{open&&<><button type="button" onClick={()=>setOpen('')}>Beispiel schließen</button>{open==='allowances'?<AllowanceDashboard data={allowanceExample} onBack={()=>setOpen('')}/>:<StatisticsDashboard data={statisticsExample}/>}</>}</>;
}
