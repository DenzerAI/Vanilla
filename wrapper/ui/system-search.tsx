...
export function SystemSearch({api, pages, onOpen}: Props) {
  ...
  const labels: Record<string,string> = {chat:'Chats',file:'Ergebnisse',knowledge:'Wissen und Notizen',job:'Aufträge',skill:'Skills',project:'Workspaces',page:'Bereiche',setting:'Einstellungen'};
  ...
  return <div className="system-search-content">
    ...
    <div ref={list} className="system-search-list"><ScrollEdgeFade className="system-search-results" aria-label="Suchergebnisse" aria-busy={loading} onKeyDown={event => {
      ...
      {loading&&!results.length&&!error&&<Skeleton layout="search" rows={3} announce={false}/>}
