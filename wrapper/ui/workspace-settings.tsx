...
export function WorkspaceSettingsPreview(){
  ...
  return <><WorkspaceInfo/><WorkspaceDefinitionEditor api={api} projectId="example" onSaved={setSaved} onCancel={()=>setSaved({project:{name:'Abgebrochen'}})}/><FilterPicker label="Workspace" value={workspace} onChange={setWorkspace} options={[{value:"all",label:"Alle Workspaces"},{value:"default",label:"Allgemein"},{value:"marketing",label:"Marketing"}]}/><p className="page-note">Lokales Beispiel. {saved?'Auswahl: '+saved.project.name:'Es werden keine Workspaces angelegt.'}</p></>;
}
