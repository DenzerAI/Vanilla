...
/** @param {{requests?: any[], notifications?: any[], chats?: any[], projectId?: string, jobs?: any[], entries?: any[], reports?: any[], now?: number, includeWeather?: boolean}} options
 * @returns {any[]} */
export function chatStartFeed({requests=[],notifications=[],chats=[],projectId='default',jobs:scheduledJobs=[],entries=[],reports=[],now=Date.now(),includeWeather=false}={}) {
  ...
  if(includeWeather)chosen.push({id:'weather',kind:'weather',title:'Wetter',description:'Dein Ort ist noch nicht eingerichtet.',prompt:'Ich möchte mein Wetter im Einstieg sehen. Lass uns zuerst Ort und Wetterquelle einrichten.'});
  return chosen;
}
...
export function headlineForItem(item, fallback) {
 ...
 return ({request:'Deine Rückmeldung wird gebraucht.',notice:'Ein Hinweis braucht Aufmerksamkeit.',report:'Ein neues Ergebnis liegt bereit.',artifact:'Dein letztes Erzeugnis ist griffbereit.',job:'Der nächste Auftrag ist geplant.',weather:'Dein Wetter gehört hier dazu.',chat:'Eine neue Antwort wartet auf dich.',prompt:'Was möchtest du voranbringen?'})[item.kind] || fallback;
}
export function headlinesForItem(item, fallback) {
 const first=headlineForItem(item,fallback);
 if(!item)return [first];
 const detail=({request:'Öffne die Rückfrage, damit die Arbeit weitergehen kann.',notice:previewText(item.description),report:previewText(item.description),artifact:'Öffne die Datei zum Ansehen oder Weiterarbeiten.',job:item.description,weather:'Dafür fehlen noch dein Ort und eine Wetterquelle.',chat:'Im Gespräch findest du das Ergebnis und kannst direkt daran anknüpfen.',prompt:'Beschreibe kurz dein Ziel. Den nächsten Schritt planen wir gemeinsam.'})[item.kind];
