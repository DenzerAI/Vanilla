...
type Props = {title?:string;text:string;chatId:string;messageId:string;disabled?:boolean;api:(url:string,data?:unknown)=>Promise<any>;Button:React.ElementType};
export function MessageSpeech({title,text,chatId,messageId,disabled,api,Button}:Props) {
  ...
  return <Button label={active?'Vorlesen stoppen':'Antwort vorlesen'} active={active} aria-pressed={active} disabled={!active&&(disabled||!spokenText(text))} onClick={()=>active?chatAudio.stop():void chatAudio.start(api,{chatId,title:title||'Antwort',key:messageId,text}).catch(error=>chatAudio.fail(error))} data-capability="chat.message.read-aloud"><Icon size={15} strokeWidth={1.55}/></Button>;
}
