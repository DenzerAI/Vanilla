...
import { RefreshCw, X } from './icons.jsx';
import { reconnectEventStream } from './chat-events.mjs';
import './system-notice.css';

export function SystemNotice({api, message, onDismiss}) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (deadline.current && next.instanceId !== initialInstance.current) {
  ...
  return <>
    {visible && <div className="system-notice" role="status" aria-live="polite">
      {(restart || reload || busy) && <RefreshCw size={16} aria-hidden="true" />}
      <span className="system-notice-text">{authRequired ? 'Bitte erneut anmelden.' : error || (busy ? 'Server wird neu gestartet …' : restart ? 'Server-Neustart erforderlich' : reload ? 'Aktualisierung verfügbar' : message)}</span>
      {authRequired && <button onClick={()=>setLoginOpen(true)}>Anmelden</button>}
      {!authRequired && !busy && (restart || reload) && <button onClick={()=>restart ? restartServer() : reloadPage()}>{restart ? 'Neu starten …' : 'Neu laden'}</button>}
