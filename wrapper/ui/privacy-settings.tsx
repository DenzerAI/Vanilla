...
export function PrivacySettings({api, onSettings, onSearch}: Props) {
  ...
  async function save() {
    const missing = status?.requirements.find(item => {
      const review = draft?.values.reviews[item.id];
      return review && review.status !== 'open' && review.note.trim().length < 10;
    });
    if (missing) throw new Error(`Bitte ergänze bei „${missing.title}“ einen kurzen Nachweis oder eine Begründung.`);
