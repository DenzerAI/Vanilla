...
export class SkillLibrary {
  ...
  async copy({id,name,version}) {
    const run=this.queue.catch(()=>{}).then(async()=>{
      ...
      name=safeName(name);const parent=await inside(this.store.root,'skills'),target=path.join(parent,name);
  ...
  async proposal({name,description,content}) {
    ...
    const parent=await inside(this.store.root,'skills'),dir=path.join(parent,name);
