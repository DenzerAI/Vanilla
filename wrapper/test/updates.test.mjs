...
test('HTTP restart is token protected and replaces the server at the same address', {timeout:30000}, async()=>{
  ...
  const dir=await mkdtemp(path.join(os.tmpdir(),'restart-http-'));
  const reservation=net.createServer();reservation.listen(0,'127.0.0.1');await once(reservation,'listening');
  const port=reservation.address().port;await new Promise(r=>reservation.close(r));
  const child=spawn(process.execPath,['server.mjs'],{cwd:new URL('..',import.meta.url),env:{...process.env,UWE_PORT:String(port),UWE_WORKSPACE:path.join(dir,'workspace'),UWE_DATA_ROOT:path.join(dir,'data')},stdio:'ignore'});
