...
async function scenario({fail=false,busy=false,upload=false}={}) {
 ...
 const env={text:draft,attachments:files,busy,running:false,nextSelection:null,pickerModel:'test-model',pickerEffort:'medium',mode:'default',chatId:'test-chat',draftKey:()=> 'test-chat',uploadCounts:{current:new Map(upload?[['test-chat',1]]:[])},setText:v=>{draft=typeof v==='function'?v(draft):v;},setAttachments:v=>{files=v;},setBusy:()=>{},notify:()=>{},followScroll:{current:false},chatRef:{current:'test-chat'},setThread:()=>{},active:{},inputRef:{current:null},api:async(path,body)=>{calls.push({path,body});if(fail)throw new Error('Send failed');return {};}};
