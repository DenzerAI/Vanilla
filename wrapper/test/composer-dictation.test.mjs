...
async function scenario({fail=false,busy=false,upload=false,question=false}={}) {
 ...
 const env={submitMessage:(api,id,body)=>submitMessage(api,id,body,storage),questionState:{request:question?{id:1}:null,text:'',submit:async text=>replies.push(text)},text:draft,attachments:files,busy,running:false,nextSelection:null,pickerModel:'test-model',pickerEffort:'medium',mode:'default',chatId:'test-chat',draftKey:()=> 'test-chat',uploadCounts:{current:new Map(upload?[['test-chat',1]]:[])},setText:v=>{draft=typeof v==='function'?v(draft):v;},setAttachments:v=>{files=v;},setBusy:()=>{},notify:()=>{},followScroll:{current:false},chatRef:{current:'test-chat'},setThread:()=>{},active:{},inputRef:{current:null},api:async(path,body)=>{calls.push({path,body});if(fail)throw new Error('Send failed');return {};}};
