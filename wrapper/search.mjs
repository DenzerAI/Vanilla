...
export async function searchConversations({workspace, chats, projects = [], threadCache = new Map(), query = ''}) {
  ...
  async function scan() {
    while (cursor < candidates.length) {
      ...
      if (query) {
        if (!thread) {
          try { thread = JSON.parse(await readFile(path.join(workspace, 'chats', chat.id, 'transcript.json'), 'utf8')); }
          catch { unavailable++; }
        }
