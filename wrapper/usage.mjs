...
function windowRow(id,name,value,now,claude=false){
 ...
 return {id,label:name,usedPercent:percent,resetAt,expired:resetAt!==null&&resetAt<=now};
}
export function codexAllowance(data,now=Date.now()) {
 ...
 for(const [id,b] of Object.entries(buckets)){
  ...
  for(const key of ['primary','secondary']){
   ...
   const row=windowRow(`${id}:${key}`,`${label(b.limitName)||'Codex'} · ${period}`,w,now);if(row)rows.push(row);
  }
 }
...
export function claudeAllowance(data,now=Date.now()) {
 ...
 for(const [key,name] of [['five_hour','Claude · 5 Stunden'],['seven_day','Claude · Woche'],['seven_day_oauth_apps','Claude · Apps'],['seven_day_opus','Opus · Woche'],['seven_day_sonnet','Sonnet · Woche']]){
  if((limits?.model_scoped||[]).some(m=>`${label(m.display_name)} · Woche`===name))continue;
  const row=windowRow(key,name,limits?.[key],now,true);if(row)rows.push(row);
 }
 for(const [i,m] of (limits?.model_scoped||[]).entries()){const row=windowRow(`model:${i}`,`${label(m.display_name)} · Woche`,m,now,true);if(row)rows.push(row);}
