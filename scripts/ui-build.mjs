...
export async function recordUiBuild(root = repositoryRoot, uiVersion = undefined) {
  ...
  const manifest = {format:1, uiVersion:uiVersion || await uiSourceVersion(root), sourceRevision:revision.status===0?revision.stdout.trim():null, sourceDirty:status.status===0?!!status.stdout.trim():null, files};
...
export async function verifyUiBuild(root = repositoryRoot) {
  ...
  return {ok:true, uiVersion:manifest.uiVersion, sourceRevision:manifest.sourceRevision, sourceDirty:manifest.sourceDirty};
}
