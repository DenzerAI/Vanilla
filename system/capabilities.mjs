...
export const capabilities = {
  ...
  actions: [
    ...
    {id:'chat.start', surface:'default entry and new chat', component:'wrapper/ui/chat-start.tsx', api:{notifications:'GET /api/notifications',reportChat:'POST /api/planner/chat'}, behavior:'Contextual greeting and compact attention fan; questions before unread latest routine results and completed chats; starter cards fill the composer only; existing chat and approval routes; no auto-send', contract:'wrapper/surfaces/chat.md'},
    {id:'planner.overview', surface:'search and direct links ?view=today or ?view=calendar; legacy pipeline link opens today', component:'wrapper/ui/planner.tsx', selector:'[data-capability="planner.overview"]', api:{schema:'GET /api/crm/schema',crm:'POST /api/crm/query',entity:'GET /api/crm/entities/{entity_id}',notifications:'GET /api/notifications'}, behavior:'Today briefing, agenda, decisions; day/week/month calendar with ISO weeks and optional weekdays; labelled isolated interactive examples, real CRM due steps and existing notifications; no external calendar sync, geolocation, weather fetch or persistent event writes', contract:'docs/PLANNER.md'},
