import {mailCapabilities} from './mail-capabilities.mjs';
...
export async function inventory(root = fileURLToPath(new URL('../', import.meta.url))) {
  ...
  for(const dir of ['wrapper','core']) {
    for(const file of (await readdir(path.join(root,dir))).sort()) {
      ...
      const patterns = [ /\broute\(\s*['"](GET|POST|PUT|PATCH|DELETE)['"]\s*,\s*['"]([^'"]+)['"]/g, /@(?:app|router)\.(get|post|put|patch|delete)\(\s*['"]([^'"]+)['"]/g ];
  ...
  return {...capabilities, endpointCoverage:'All literal route(...) and @app/@router.method(...) declarations in wrapper/*.mjs and core/*.py; dynamic routes and external worker tools remain in referenced catalogs.', endpoints};
}
