...
export const capabilities = {
  ...
  domains: [
    ...
    {id:'pipeline', sources:['wrapper/ui/pipeline.tsx','wrapper/ui/pipeline.css'], contract:'wrapper/surfaces/pipeline.md', status:'design-preview-only', backend:null},
  ...
  actions: [
    ...
    {id:'pipeline.preview', surface:'main navigation after inbox; ?view=pipeline', component:'wrapper/ui/pipeline.tsx', selector:'[data-capability="pipeline.preview"]', api:null, behavior:'Fictional local entries; stage changes require next action and date while open; no persistent customer data or connector writes'},
    {id:'chat.model.select', surface:'composer model popover', component:'wrapper/ui/model-picker.jsx', sources:['wrapper/worker-models.mjs','system/worker-catalog.mjs'], api:{activate:'POST /api/workers/activate', create:'POST /api/chats', configure:'POST /api/worker-session', nextTurn:'POST /api/turn', status:'GET /api/workers'}, behavior:'Explicit Codex/Claude Code selection; native CLI/OAuth auth; no worker restart; GPT-5.6/6 UI filter; exact advertised effort values with a terracotta step slider; per-chat next-turn selections leave active turns unchanged; confirmed ACP configs; provider switch starts a new chat and preserves history'},
