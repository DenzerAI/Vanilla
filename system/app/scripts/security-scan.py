...
FORBIDDEN = {'knowledge', 'memory', 'data', 'workspaces', 'firmenbasis', 'brain', 'soul', 'notes', 'input',
...
def source_path(name):
    if name.startswith('system/app/'):
        name = name.removeprefix('system/app/')
...
class Scanner:
    ...
    def policy_snapshot(self, revision):
        try:
            raw = git(self.root,'show',revision+':system/app/system/source-policy.json')
        except ValueError:
            raw = git(self.root,'show',revision+':system/source-policy.json')
    ...
    def entry(self, name, raw, mode='100644', historical=False, revision=None):
        logical_name = name.removeprefix('system/app/')
        ...
        if not source_path(name):
            ...
            return
        if logical_name.startswith('templates/') and digest != self.policy['neutralTemplates'].get(logical_name):
            self.add('changed-neutral-template', name, revision=revision)
        if PurePosixPath(name).suffix in {'.png', '.woff2', '.svg'}:
            if digest != self.policy['reviewedBinaryAssets'].get(logical_name):
        ...
        try:
            ...
            return
        self.text(logical_name, raw, revision)
