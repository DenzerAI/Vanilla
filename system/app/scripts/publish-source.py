...
SOURCE = ROOT
PREFIX = 'system/app/' if ROOT.name == 'app' and ROOT.parent.name == 'system' else ''
if PREFIX: ROOT = ROOT.parents[1]
...
def source_files(root=ROOT):
    ...
    files = []
    forbidden = {'.git', '.venv', 'node_modules', 'data', 'knowledge', 'memory', 'workspaces', '.cache', '.verify',
    ...
    for name in sorted(set(filter(None, names))):
        ...
        if (name in {'IDENTITY.md','system/layout.json'} or name.startswith(('firmenbasis/','system/migrations/')) or set(Path(name).parts) & forbidden or path.is_symlink()
...
def export(destination):
    ...
    run("npm", "--prefix", PREFIX + "wrapper", "run", "design:verify", capture=False)
    files = source_files()
    for name in files:
        target = destination / name
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / name, target)
    run(sys.executable, PREFIX + 'scripts/security-scan.py', '--directory', '.', cwd=destination, capture=False)
