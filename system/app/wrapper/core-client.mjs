...
export function recordKey(file) {
  ...
  const portable = process.env.VANILLA_LAYOUT === '2' ? relative.replace(/^workspaces\/[^/]+\//,'') : relative;
  if (/^(chats\/[a-zA-Z0-9_-]+\/(transcript|tools)\.json|projects\/[a-zA-Z0-9_-]+\/project\.json|jobs\/[a-zA-Z0-9_-]+\/runs\/[a-zA-Z0-9_-]+\/(request|result)\.json)$/.test(portable)) return "workspace/" + portable;
  return null;
}
