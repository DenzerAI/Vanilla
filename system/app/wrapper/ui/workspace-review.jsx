...
export function ReviewPanel({api, projectId, projectName}) {
  ...
  return <div className="workspace-review">
    <div className="review-toolbar"><GitBranch size={16}/><span>{result?.branch || projectName || 'Workspace'}</span><button className="icon-button" aria-label="Änderungen aktualisieren" disabled={loading} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={16}/></button></div>
