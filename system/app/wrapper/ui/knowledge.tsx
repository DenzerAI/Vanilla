  ...
  return <section className="knowledge-panel" aria-label="Lokales Wissen">
    ...
    <div className="row between library-toolbar"><p className="skill-results-status" role="status">{busy?'Suche läuft …':`${hits.length} Fundstellen`}</p>
      <div className="row"><button aria-label="Wissensindex aktualisieren" disabled={busy} onClick={async()=>{setBusy(true);setError('');try{const r=await api('/knowledge/reindex',{});setRevision(v=>v+1);notify(`${r.documents} Textdateien erfasst.`);}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}><RefreshCw size={18} strokeWidth={1.55}/></button>
        <button onClick={()=>{const id=project==='all'?'default':project;const p=projects.find(p=>p.id===id);setNote({path:(p?.path?p.path+'/':'')+(p?.path?.startsWith('workspaces/')?'knowledge/':'notes/'),projectId:id,isNew:true});}}><Plus size={18} strokeWidth={1.55}/>Notiz erstellen</button></div></div>
...
export function NoteEditor({api,entry,onClose,onSaved,onOpen}:{api:Api;entry:{path:string;projectId:string;isNew?:boolean};onClose:()=>void;onSaved:()=>void;onOpen:(p:string)=>void}) {
  ...
  return <Modal title={entry.isNew?'Notiz erstellen':entry.path.split('/').pop()} wide onClose={close} className="knowledge-note">
    ...
    {note&&<form className="grid knowledge-gap" onSubmit={async e=>{e.preventDefault();setBusy(true);setError('');try{
      ...
      {!entry.isNew&&<div className="knowledge-links grid knowledge-gap"><div><strong className="row"><Link size={16} strokeWidth={1.55}/>Verlinkt mit</strong><div className="flex flex-wrap knowledge-gap-small">{note.links.length?note.links.map((link,i)=> <button type="button" key={i} disabled={!link.path||dirty} onClick={()=>link.path&&onOpen(link.path)}>{link.label||link.target}{!link.path&&(link.ambiguous?' · mehrdeutig':' · noch nicht vorhanden')}</button>):<p className="page-note">Noch keine Verknüpfungen.</p>}</div></div>
        <div><strong>Verweise auf diese Notiz</strong><div className="flex flex-wrap knowledge-gap-small">{note.backlinks.length?note.backlinks.map(link=><button type="button" key={link.path} disabled={dirty} onClick={()=>onOpen(link.path)}>{link.title}</button>):<p className="page-note">Noch keine Rückverweise.</p>}</div></div></div>}
    </form>}
  </Modal>;
}
