...
import {ArrowLeft,Folder,FileText,RefreshCw,Download,Lock,ChevronRight,Eye,EyeOff} from './icons.jsx';
...
export function AgentFiles({api,initialFolder,onPreview}) {
  ...
  return <>
    ...
    {file?<div className="workspace-file-list"><FileContent key={file} path={file} api={api} scope="agent" readOnly onEnlarge={onPreview?()=>onPreview(file):undefined}/></div>:<>
      {path && <div className="workspace-file-path" title={[data?.root,path].filter(Boolean).join('/')}>{[rootName,...path.split('/').filter(Boolean).slice(0,-1)].join(' › ')}</div>}
      ...
      {data && !error && <div className="workspace-file-footer"><span>{visibleEntries.length} Einträge</span>{protectedCount > 0 && <button className="icon-button" aria-label={showProtected ? 'Geschützte Einträge ausblenden' : 'Geschützte Einträge einblenden'} title={`${protectedCount} geschützte Einträge · ${showProtected ? 'ausblenden' : 'einblenden'}`} aria-pressed={showProtected} onClick={()=>setShowProtected(value=>!value)}>{showProtected ? <Eye size={16}/> : <EyeOff size={16}/>}</button>}</div>}
    </>}
  </>;
}
