        ...
        {view === "inbox" ? (
          ...
          <>
            ...
            <div className="workspace-projects">
              <div className="sidebar-section-label projects-heading"><span>Workspaces</span><IconButton label="Neuer Workspace" onClick={()=>setModal({type:"project"})}>{icon(Plus,16)}</IconButton></div>
                    ...
                    {panel === "files" ? (
                      ...
                      <div className="terminal-panel">
                        {terminalOutput ? <pre aria-label="Befehlsausgabe">{terminalOutput}</pre> : <div className="workspace-command-empty">
                          ...
                          <p>Einzelne Befehle im Workspace-Ordner.</p>
        ...
        >
          <form className="project-editor"
            onSubmit={guard(async (e) => {
              ...
              await saveProject({ id: modal.project?.id, name, icon: new FormData(e.currentTarget).get("icon"), color: new FormData(e.currentTarget).get("color"), knowledge: new FormData(e.currentTarget).getAll('knowledge') });
            })}
          >
      ...
      {modal?.type === "job-run" && (
        <Modal title={modal.job.name} onClose={() => setModal(null)}>
          ...
          <div className="row end">
            <button
              onClick={() => {
                setAgentFolderTarget(`${boot.workspace}/${modal.job.path || `jobs/${modal.job.id}`}/runs`);
                setSelectedFile(null);
                setPanel("files");
                setView("chat");
                setModal(null);
