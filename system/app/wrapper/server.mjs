...
import {installationRoot, sourceRoot, modernLayout} from './layout.mjs';
...
const sourceVersion = () => serverFingerprint(sourceRoot);
...
const workspace = modernLayout ? root : localPath(process.env.UWE_WORKSPACE || "workspaces/default");
...
async function finishThread(id, turn) {
  ...
  if (c.jobId) {
    ...
    if (job) {
      ...
      await atomic(
        path.join(await store.jobRoot(job.id), "runs", c.runId, "result.json"),
        result,
      );
      await atomic(
        path.join(await store.jobRoot(job.id), "output", "latest.md"),
...
async function sendTurnUnlocked(id, b) {
  ...
  for (const attachment of b.attachments || []) {
    const file = await store.filePath(attachment.path);
  ...
  const companyContext = await workerInstructions({ root, workspace, cwd: c.cwd, project: store.project(c.projectId || 'default') })
...
route("GET", "/api/bootstrap", async () => {
  ...
  return {
    token,
    identitySource: modernLayout ? "IDENTITY.md" : "soul/IDENTITY.md",
    workspaceToolsVersion: 1,
    layoutVersion: modernLayout ? 2 : 1,
...
route("POST", "/api/projects/save", async (b) => {
  const project = await store.saveProject({ id: b.id, name: b.name, icon: b.icon, color: b.color, knowledge: b.knowledge });
  loaded.clear?.();
...
route("GET", "/api/search", async (b, u) => searchConversations({
  workspace, chats:store.state.chats, projects:store.state.projects, threadCache, chatFile: (id,...parts)=>store.chatFile(id,...parts),
...
route("GET", "/api/thread", async (b, u) => {
  ...
  try {
    ...
    const cached =
      threadCache.get(id) ||
      (await jsonFile(
        store.chatFile(id, "transcript.json"),
        null,
      ));
...
route("POST", "/api/turn/delete", async (b) => {
  ...
  const records = (toolsByThread.get(b.id) || await jsonFile(store.chatFile(b.id, "tools.json"), []))
  ...
  await atomic(store.chatFile(b.id, "tools.json"), records);
...
route("POST", "/api/fork", async (b) => {
  ...
  await atomic(
    store.chatFile(r.thread.id, "tools.json"),
    inheritedTools,
  );
  await store.save();
  await store.exportThread(r.thread);
...
const readableFile = u => u.searchParams.get("scope") === "agent" ? agentFilePath(root,u.searchParams.get("path") || "") : u.searchParams.get("scope") === "artifacts" ? library.resolve(u.searchParams.get("path") || "",'artifacts') : store.filePath(u.searchParams.get("path") || "");
...
route("POST", "/api/file/save", async (b) => {
  const file = await store.filePath(b.path);
...
route("GET", "/api/identity", async () => {
  const source = await readFile(store.identityFile, "utf8");
...
async function runJob(id, slot = null, coreRunId = null) {
  ...
  try {
    ...
    const jobDir = await store.jobRoot(id);
