...
export async function saveHandoff(store, id, thread) {
  const file = store.chatFile(id, 'handoffs', randomUUID() + '.json');
...
export async function handoffSnapshot(store, chat) {
  ...
  const file = await store.filePath(chat.handoffSnapshot);
  const directory = await store.filePath(path.relative(store.root,store.chatFile(chat.id,'handoffs')));
  if (!file.startsWith(directory + path.sep)) throw new Error('Ungültiger Übergabeverlauf.');
...
export async function handoffInstructions(store, chat) {
  ...
  await handoffSnapshot(store,chat);
  const file = (await store.filePath(chat.handoffSnapshot)).replace(/\.json$/, '.md');
