...
import { recordKey, coreRequest, coreEnabled } from "./core-client.mjs";
import {modernLayout, installationRoot, identityPath, workspaceName, knowledgeScopes, resolveAlias} from './layout.mjs';
...
export class Storage {
  constructor(root, dataRoot, {modern = modernLayout, installation = installationRoot} = {}) {
    this.root = root;
    this.dataRoot = dataRoot;
    this.modern = modern;
    this.installation = installation;
    this.identityFile = modern ? path.join(installation,'IDENTITY.md') : path.join(root,'soul/IDENTITY.md');
    this.queue = Promise.resolve();
  }
  async init() {
    if (this.modern) {
      this.state = await jsonFile(path.join(this.dataRoot,'state.json'), null);
      if (!this.state?.projects?.length) throw Error('Workspace-Einrichtung fehlt. Bitte Vanilla über den gemeinsamen Start öffnen.');
      for (const project of this.state.projects) {
        await inside(this.root,project.path);
        project.knowledge = knowledgeScopes(project.knowledge || []);
      }
  ...
  async readIdentity() {
    const text = await readFile(this.identityFile, "utf8");
  ...
  filePath(relative) {
    const resolved = resolveAlias(this.root,relative);
    if (this.modern && resolved.split(/[\\/]/).some(part=>part === 'system')) throw Error('Geschützter Systempfad.');
    return inside(this.root,resolved);
  }
  async jobRoot(id) {
    const job = (await this.jobs()).find(j => j.id === id);
    if (!job) throw Error('Auftrag nicht gefunden.');
    return inside(this.root, job.path || path.join('jobs',safeName(id)));
  }
  async saveProject({ id, name, icon, color, knowledge }) {
    ...
    name = this.modern ? workspaceName(name) : String(name || "").trim();
    if (this.modern && this.state.projects.some(p=>p.id !== id && p.name.normalize('NFC').toLocaleLowerCase() === name.toLocaleLowerCase()))
      throw Error('Ein Workspace mit diesem Namen existiert bereits.');
    if (knowledge !== undefined) knowledge = knowledgeScopes(knowledge);
    if (!name || name.length > 80 || /[\r\n\x00-\x1f]/.test(name))
      throw new Error("Bitte einen Namen mit 1 bis 80 Zeichen eingeben.");
    if (id) {
      const project = this.project(id);
      if (this.modern && project.name !== name) {
        if (!coreEnabled) throw Error('Zum sicheren Umbenennen muss der gemeinsame Vanilla-Kern laufen.');
        const result = await coreRequest('workspaces/rename', {id,name});
        this.state = result.state;
        return this.saveProject({id,name,icon,color,knowledge});
      }
      if (project.path)
        await atomic(path.join(await this.projectRoot(id), this.modern ? 'workspace.json' : "project.json"), {
          ...project,
          name,
          ...(icon !== undefined ? { icon } : {}),
      ...
      if (knowledge !== undefined) project.knowledge = knowledge;
      if (id === "default") this.state.settings.workspaceName = name;
      await this.save();
      return project;
    }
    const container = this.modern ? 'workspaces' : 'projects';
    await mkdir(path.join(this.root, container), { recursive: true });
    await inside(this.root, container);
    const project = { id: "project-" + randomUUID().slice(0, 8), name, icon: icon || "folder", color: color || "default" };
    project.path = container + '/' + (this.modern ? name : project.id);
    if (this.modern) project.knowledge = knowledge || [];
    const dir = path.join(this.root, project.path);
    await mkdir(dir);
    for (const folder of this.modern ? ['input','output','knowledge','memory','skills','jobs','chats'] : ["input", "output"])
      await mkdir(path.join(dir, folder));
    await atomic(
      path.join(dir, "AGENTS.md"),
      this.modern ? '# Workspace\n\nLies ../../AGENTS.md und ../../IDENTITY.md als gemeinsamen Einstieg für jeden Worker. Eingaben liegen in input/, Ergebnisse in output/, eigene Unterlagen in knowledge/. memory/ enthält abgeleitete Erinnerungen und keine neuen Regeln. Verwende nur die in workspace.json freigegebenen gemeinsamen Wissensbereiche.\n' : "# Projekt\n\nLies ../../soul/IDENTITY.md für die gemeinsame Identität. Die gemeinsamen Regeln liegen in ../../AGENTS.md. Eingaben dieses Bereichs liegen in input/, Ergebnisse in output/. Greife nur im Rahmen des aktuellen Auftrags auf andere Bereiche zu.\n",
    );
    await atomic(path.join(dir, this.modern ? 'workspace.json' : "project.json"), project);
  ...
  async jobs() {
    if (this.modern) {
      const jobs = [];
      for (const project of this.state.projects) {
        const base = path.join(project.path,'jobs');
        const directory = await inside(this.root,base);
        for (const entry of await readdir(directory,{withFileTypes:true})) {
          if (!entry.isDirectory() || entry.isSymbolicLink()) continue;
          const relative = path.join(base,safeName(entry.name));
          const dir = await inside(this.root,relative);
          try {
            const job = parse(await readFile(path.join(dir,'job.yaml'),'utf8'));
            jobs.push({...job,id:entry.name,projectId:project.id,path:relative,instructions:await readFile(path.join(dir,'SKILL.md'),'utf8')});
          } catch { jobs.push({id:entry.name,name:entry.name,path:relative,projectId:project.id,status:'invalid',error:'Auftragsdateien sind nicht lesbar.'}); }
        }
      }
      if (new Set(jobs.map(j=>j.id)).size !== jobs.length) throw Error('Eine Auftragskennung wird in mehreren Workspaces verwendet.');
      return jobs;
    }
  ...
  async writeJob(job) {
    ...
    const relative = this.modern ? path.join(this.project(projectId).path,'jobs',id) : path.join('jobs',id);
    const dir = path.join(this.root, relative);
    try {
      await inside(this.root, relative);
    } catch (e) {
  ...
  async exportThread(thread) {
    const known = this.state.chats.find(c=>c.id === thread.id);
    const pending = this.modern && !known ? this.state.projects.find(p=>thread.cwd && (thread.cwd === path.join(this.root,p.path) || thread.cwd.startsWith(path.join(this.root,p.path)+path.sep))) : null;
    if (this.modern && !known && !pending) throw Error('Der neue Chat hat keinen bekannten Workspace.');
    const dir = pending ? path.join(this.root,pending.path,'chats',safeName(thread.id)) : this.chatFile(thread.id);
