...
def safe_path(root: Path, relative: str, *, missing=False):
    ...
    relative = alias_path(root, relative)
    pieces = Path(relative).parts
    if (
        not pieces
        or Path(relative).is_absolute()
        or any(
            p.startswith(".") or p in {"secrets", "node_modules", "data", "codex", "system"}
            for p in pieces
        )
    ):
...
class Storage:
    ...
    def import_legacy(self):
        ...
        if self.config.layout:
            patterns = tuple('workspaces/*/' + p for p in patterns if not p.startswith('projects/'))
        for pattern in patterns:
            for p in self.config.workspace.glob(pattern):
                sources.append(
                    ("workspace/" + ('/'.join(p.relative_to(self.config.workspace).parts[2:]) if self.config.layout else p.relative_to(self.config.workspace).as_posix()), p)
                )
    ...
    def sync_jobs(self):
        ...
        jobs = []
        for p in sorted(self.config.workspace.glob('workspaces/*/jobs/*/job.yaml' if self.config.layout else "jobs/*/job.yaml")):
            ...
            try:
                ...
                if self.config.layout:
                    value['path'] = p.parent.relative_to(self.config.workspace).as_posix()
                    project_path = p.parent.parent.parent.relative_to(self.config.workspace).as_posix()
                    rows = self.db.rows('SELECT id FROM projects WHERE path=?',(project_path,))
                    if not rows: raise ValueError('Auftrag liegt in einem unbekannten Workspace.')
                    value['projectId'] = rows[0]['id']
