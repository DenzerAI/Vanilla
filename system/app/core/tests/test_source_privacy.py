...
def repo(tmp_path, monkeypatch):
    ...
    paths = ['.gitignore', 'system/source-policy.json', 'backend/company-base.mjs',
             'wrapper/isolation.mjs', 'wrapper/layout.mjs', 'scripts/security-scan.py', 'scripts/source-sync.py',
    ...
    for name in paths:
        ...
        source = ROOT / name
        if not source.exists() and name.startswith('.githooks/'):
            source = ROOT.parents[1] / name
        shutil.copy2(source, target)
        if name.startswith('.githooks/'):
            target.write_text(target.read_text().replace('/system/app/scripts/', '/scripts/'))
