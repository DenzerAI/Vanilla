...
class Knowledge:
    ...
    def roots(self):
        if self.config.layout:
            roots = [('knowledge:'+scope,self.config.root/'knowledge'/scope) for scope in ['company','personal']]
            for p in self.db.rows('SELECT id,path FROM projects'):
                for name in ['knowledge','memory','notes','brain','input','output']:
                    roots.append((p['id'],self.config.root/p['path']/name))
            return roots
        roots = [
    ...
    def search(self, query, project="all", limit=30, semantic=True, _scoped=False):
        if self.config.layout and project != 'all' and not _scoped and not project.startswith('knowledge:'):
            rows = self.db.rows('SELECT data FROM projects WHERE id=?',(project,))
            if not rows: raise ValueError('Workspace nicht gefunden.')
            scopes = [s for s in json.loads(rows[0]['data']).get('knowledge',[]) if s in ['company','personal']]
            results = [r for scope in [project,*['knowledge:'+s for s in scopes]] for r in self.search(query,scope,limit,semantic,True)]
