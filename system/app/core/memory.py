...
class Memory:
    ...
    def save(self, path, text, version, project="default", kind="Notiz geändert"):
        with self.lock:
            ...
            path = file.relative_to(self.config.workspace).as_posix()
    ...
    def restore(self, revision, path, version, project):
        ...
        if self.config.layout:
            for old,new in self.config.layout.get('legacyPaths',{}).items():
                if path.startswith(new+'/'):
                    suffix = path[len(new)+1:]
                    if suffix.startswith('memory/'): suffix = 'brain/'+suffix[len('memory/'):]
                    elif suffix.startswith('knowledge/'): suffix = 'notes/'+suffix[len('knowledge/'):]
                    candidates.append('/'.join(p for p in [old,suffix] if p))
        result = next((result for candidate in candidates if (result := self.git('show',revision+':'+candidate,check=False)).returncode == 0),None)
        if result is None: raise ValueError('Die historische Datei wurde in dieser Version nicht gefunden.')
        text = result.stdout
        return self.save(path, text, version, project, "Version wiederhergestellt")

    def project_prefix(self, project):
        if project == "default" and not self.config.layout:
            return ""
    ...
    @property
    def folder(self):
        return 'memory' if self.config.layout else 'brain'
    ...
    def capture(self, chat_id):
        ...
        target = f"{prefix}{self.folder}/daily/{day}/{chat_id}.md"
        count = 0
        with self.lock:
    ...
    def continuation(self, chat_id):
        ...
        path=self.project_prefix(project)+self.folder+'/continuations/'+chat_id+'.md'
        with self.lock:
    ...
    def dream(self):
        with self.lock:
            ...
            for item in projects:
                ...
                target = prefix + self.folder + "/memory/Ergebnisse.md"
                ...
                notes = 'knowledge' if self.config.layout else 'notes'
                self.write_managed(prefix + self.folder + "/MEMORY.md", f"# Memory\n\n[[{prefix}{self.folder}/memory/Ergebnisse.md|Gesprächsergebnisse]]\n\nEigene Notizen liegen in {notes}/. Historische Auszüge liegen in {self.folder}/daily/.\n", project, "Memory-Index")
                ...
                self.write_managed(prefix + f"{self.folder}/dreams/{day}.md", report, project, "Pflegebericht")
                total += len(entries)
    ...
    def forget_chat(self, chat_id):
        with self.lock:
            ...
            if chat:
                ...
                continuation=self.project_prefix(project)+self.folder+'/continuations/'+chat_id+'.md'
            ...
            for row in rows:
                candidates.add(self.project_prefix(row['project_id'])+self.folder+'/memory/Ergebnisse.md')
            if chat:
    ...
    def context(self, query, project, chat_id=None, max_chars=None):
        ...
        rows = self.db.rows('SELECT data FROM projects WHERE id=?',(project,)) if self.config.layout else []
        company_enabled = not self.config.layout or bool(rows and 'company' in json.loads(rows[0]['data']).get('knowledge',[]))
        current = self.crm.context_sources(query, min(2500, limit // 2)) if self.crm and company_enabled else []
        ...
        if options["shared_notes"] and project != "default" and not self.config.layout:
            ...
            if left > 300:
                ...
                for hit in hits:
                    if not hit["path"].startswith(self.project_prefix('default') + ('knowledge/shared/' if self.config.layout else 'notes/shared/')):
                        continue
