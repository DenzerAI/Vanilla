...
class Backups:
    ...
    def target(self):
        ...
        protected = [self.config.root/'workspaces',self.config.root/'knowledge',self.config.data] if self.config.layout else [self.config.workspace,self.config.data]
        ...
        try:
            ...
            with self.memory.lock, self.memory.knowledge.lock, self.db.lock:
                ...
                if self.config.layout:
                    for name in ['workspaces','knowledge']:
                        copy_stable(self.config.root/name, stage/'workspace'/name)
                    for name in ['IDENTITY.md','system/layout.json']:
                        target = stage/'workspace'/name
                        target.parent.mkdir(parents=True,exist_ok=True)
                        shutil.copy2(self.config.root/name,target)
                else:
            ...
            manifest = {"format": "agent-backup-v2" if self.config.layout else "agent-backup-v1", "installation":str(self.config.root), "created_at": time(), "schema": 2, "files": files, "models": "rebuild", "secrets": "keychain-reconnect"}
    ...
    def stage_restore(self, snapshot):
        ...
        with self.lock:
            ...
            for p in destination.rglob("manifest.json"):
                try:
                    if json.loads(p.read_text()).get("format") in {'agent-backup-v1','agent-backup-v2'}:
                        candidates.append(p)
...
def verify_restore(base):
    ...
    if manifest.get("format") not in {'agent-backup-v1','agent-backup-v2'}:
    ...
    if manifest['format'] == 'agent-backup-v2' and not {'workspace/IDENTITY.md','workspace/system/layout.json'}.issubset(actual):
        raise ValueError('Identität oder Pfadzuordnung fehlen in der Workspace-Sicherung.')
