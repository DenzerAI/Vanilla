...
import {sourceRoot, modernLayout, layoutManifest} from '../wrapper/layout.mjs';

export function companyRoot(root) {
  root = realpathSync(path.resolve(root));
  const layout = layoutManifest(root);
  const configured = process.env.COMPANY_BASE;
  const legacy = configured && layout?.aliases?.[path.relative(root,configured)];
  const base = localPath(legacy ? path.join(root,legacy) : configured || path.join(root, modernLayout || layout ? 'knowledge/company' : 'firmenbasis'), root);
  ...
  } else if (!['knowledge','firmenbasis','data','workspaces'].includes(relative.split(path.sep)[0])) {
...
export async function ensureCompanyBase(root) {
  ...
  try { await access(base); }
    ...
    try {
      await cp(path.join(existsSync(path.join(root,'templates/firmenbasis')) ? root : sourceRoot, 'templates/firmenbasis'), temporary, {recursive:true, errorOnExist:true, force:false});
