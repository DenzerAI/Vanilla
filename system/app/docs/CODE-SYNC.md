...
Alle technischen Quellpfade in diesem Dokument beziehen sich auf `system/app/`.
Die Git-Wurzel enthält den gemeinsamen Bootstrap; Identität, `knowledge/`,
`workspaces/` und `system/data/` bleiben vollständig lokal.
...
und richtet die gemeinsame Wissensbasis aus `templates/firmenbasis/` ein, sofern der lokale
...
`knowledge/` und die bisherige `firmenbasis/` sind vollständig ausgeschlossen, auch alle Firmen-Arbeitsweisen.
...
- Inhalte: erkennbare Schlüssel, private Schlüsselblöcke, Kontaktadressen,
  ...
  Zeile, in `knowledge/company/private-terms.txt` hinterlegt werden.
