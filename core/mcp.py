...
ROUTINE_FIELDS = {'category':{'type':'string','maxLength':80,'description':'Optional user category; empty means Allgemein. Sorting only: never changes workspace, instructions or output location.'},'name':{'type':'string','maxLength':200},'instructions':{'type':'string','maxLength':30000,'description':'Self-contained task, data sources and desired result. Runs have no implicit access to this chat history.'},'schedule':SCHEDULE,'notification':{'type':'object','properties':{'target':{'type':'string','description':'app or an exact ready target ID from routine_capabilities; external delivery only on explicit user request'},'when':{'type':'string','enum':['always','errors']}},'required':['target','when'],'additionalProperties':False},'status':{'type':'string','enum':['active','paused']}}
DEFINITIONS += [
