...
WRITE_TOOLS = {'memory_write','crm_capture','crm_propose','routine_create','routine_update','device_action'}
...
def call_core(args, name, arguments):
    ...
    family = 'devices' if name.startswith('device_') else 'routines' if name.startswith('routine_') else 'crm' if name.startswith('crm_') else 'memory'
