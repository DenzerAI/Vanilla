...
def install(config, activate=False):
    ...
    for suffix, args, options in [
        ...
        definition = {"Label": name + suffix, "ProgramArguments": args, "WorkingDirectory": str(config.root), "EnvironmentVariables": environment, "StandardOutPath": str(logs / ("heartbeat.log" if suffix else "core.log")), "StandardErrorPath": str(logs / ("heartbeat.log" if suffix else "core.log")), "Umask": 0o077, **options}
